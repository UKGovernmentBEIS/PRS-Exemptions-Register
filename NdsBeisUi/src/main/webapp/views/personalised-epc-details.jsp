...
<fmt:message bundle="${FieldsBundle}"
	key="Title_Energy_Performance_Certificate" var="title" />
			...
			<div class="column-full">
				<c:choose>
					<c:when
						...
						<c:set var="showaddbutton"
							value="${fn:length(command.exemptionDetails.epc.files.resources) == 0 }" />
					</c:when>
					<c:otherwise>
                ...
                <div class="form-group">	
					...
					<jsp:include page="personalised-exemption-upload-component.jsp">
						<jsp:param name="path"
							value="exemptionDetails.epc.files" />
						...
						<jsp:param name="uipath" value="uiData.multipartFile" />
							...
							<table id="uploadedFiles">
								...
								<c:forEach
									...
									<tr>
										...
										<td><nds:input
												path="exemptionDetails.epc.files.resources[${res.index}].description" class="form-control"/></td>
										<td class="removefile">
											<button type="submit" name="action" role="link"
												id="DeleteResource:${command.exemptionDetails.epc.files.resources[res.index].fileId}"
												value="DeleteResource:${command.exemptionDetails.epc.files.resources[res.index].fileId}">
												<fmt:message bundle="${FieldsBundle}"
