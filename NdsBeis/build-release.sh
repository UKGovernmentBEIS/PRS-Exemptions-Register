...
# Nds Platform Build
grep -l -r --include "pom.xml" "<version>(,1.0.0]</version>" nds/NdsPlatform* | xargs -i -n 1 sed 's~<version>(,1.0.0]</version>~<version>'${release_version}'</version>~g' -i {}
grep -l -r --include "pom.xml" "<version>(,1.0.0]</version>" nds/NdsMavenPlugins* | xargs -i -n 1 sed 's~<version>(,1.0.0]</version>~<version>'${release_version}'</version>~g' -i {}
...
${maven} -f nds/NdsPlatform/pom.xml -s beis/NdsBeis/release-settings.xml -DaltDeploymentRepository=build::default::${release_repo} -DdeployAtEnd=true clean install deploy:deploy -U ${plugin_version} ${platform_version}
...
${maven} -f nds/NdsMavenPlugins/pom.xml -s beis/NdsBeis/release-settings.xml versions:update-property versions:update-property -DnewVersion="[${release_version}]" -Dproperty=nds.platform.version -DgenerateBackupPoms=false
${maven} -f nds/NdsMavenPlugins/pom.xml -s beis/NdsBeis/release-settings.xml -DaltDeploymentRepository=build::default::${release_repo} -DdeployAtEnd=true clean install deploy:deploy -U ${plugin_version} ${platform_version}

# BEIS Build
grep -l -r --include "pom.xml" "<version>(,1.0.0]</version>" beis/NdsSharedPoms* | xargs -i -n 1 sed 's~<version>(,1.0.0]</version>~<version>'${release_version}'</version>~g' -i {}
grep -l -r --include "pom.xml" "<version>(,1.0.0]</version>" beis/NdsBeis* | xargs -i -n 1 sed 's~<version>(,1.0.0]</version>~<version>'${release_version}'</version>~g' -i {}
...
${maven} -f beis/NdsSharedPoms/pom.xml -s beis/NdsBeis/release-settings.xml versions:update-property versions:update-property -DnewVersion="[${release_version}]" -Dproperty=nds.platform.version -DgenerateBackupPoms=false
${maven} -f beis/NdsSharedPoms/pom.xml -s beis/NdsBeis/release-settings.xml versions:update-property versions:update-property -DnewVersion="[${release_version}]" -Dproperty=nds.beis.version -DgenerateBackupPoms=false
${maven} -f beis/NdsSharedPoms/build/pom.xml -s beis/NdsBeis/release-settings.xml -DaltDeploymentRepository=build::default::${release_repo} -DdeployAtEnd=true clean install deploy:deploy -U ${plugin_version}  ${platform_version}
