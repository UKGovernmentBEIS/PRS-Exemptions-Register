...
public class LandlordNameSearchValidator extends AbstractFieldValidator{

    public static final String DEFAULT_MESSAGE = "Validation_Field_must_not_outside_length_restrictions";
    ...
    public List<BaseValidationProperty> getProperties() {
        ...
        properties.add(new SimpleValidationProperty().setParameter("minLength").setValue(String.valueOf(minLength), null));
        properties.add(new SimpleValidationProperty().setParameter("maxLength").setValue(String.valueOf(maxLength), null));
        return properties;
    }
    ...
    public CustomValidator getCustomValidator() {
        return new CustomValidator() {
            ...
            public boolean validate(FieldValidator validator, Field field, ValidationContext<?> validationContext) {
                ...
                return (length != count || length == 0) && (lengthWithoutWildcard == 0)
                        || ((lengthWithoutWildcard >= minLength) && (lengthWithoutWildcard <= maxLength));
